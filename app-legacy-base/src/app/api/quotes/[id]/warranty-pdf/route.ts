...
import { generateWarrantyPdf } from '@/lib/pdf/warrantyPdf';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    return withAuth(request, async () => {
        ...
        try {
            const pdf = await generateWarrantyPdf(quote, getOriginFromRequest(request));
            ...
            const formattedTitle = formatDocTitle(quote.name, '保固證明書');
            ...
            return new NextResponse(pdf as any, {
                headers: {
                    ...
                    'Content-Disposition': `attachment; filename="${quote.quoteNumber}-warranty.pdf"; filename*=UTF-8''${encodedFileName}`
                }
            });
        } catch (error: any) {
