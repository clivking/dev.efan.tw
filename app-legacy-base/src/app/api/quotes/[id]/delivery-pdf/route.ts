...
import { generateDeliveryPdf } from '@/lib/pdf/deliveryPdf';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    return withAuth(request, async () => {
        ...
        try {
            const pdf = await generateDeliveryPdf(quote, getOriginFromRequest(request));
            ...
            const formattedTitle = formatDocTitle(quote.name, '出貨單');
            ...
            return new NextResponse(pdf as any, {
                headers: {
                    ...
                    'Content-Disposition': `attachment; filename="${quote.quoteNumber}-delivery.pdf"; filename*=UTF-8''${encodedFileName}`
                }
            });
        } catch (error: any) {
